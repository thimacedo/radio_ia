...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
SETUP DA OPÇÃO A — CallMeBot (5 minutos)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. Abra o WhatsApp no seu celular
2. Adicione o contato: +34 644 59 62 61 (CallMeBot)
3. Envie a mensagem: "I allow callmebot to send me messages"
4. Aguarde a resposta com sua apikey (chega em ~2 minutos)
5. Cole a apikey no .env em WA_CALLMEBOT_APIKEY
6. Pronto. Sem conta, sem cadastro, sem servidor.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
SETUP DA OPÇÃO B — Evolution API (Docker)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. Instale o Docker Desktop para Windows
2. Crie o arquivo docker-compose.yml (incluído no final deste arquivo)
3. Execute: docker compose up -d
4. Acesse http://localhost:8080 e crie uma instância
...
def _ler_env(chave: str, fallback: str = "") -> str:
    """Lê variável do .env sem depender do python-dotenv."""
...
class CallMeBotNotifier:
    ...
    def __init__(self):
        self.numero = _ler_env("WA_NUMERO")
        self.apikey = _ler_env("WA_CALLMEBOT_APIKEY")

        if not self.numero or not self.apikey:
            raise ValueError(
                "[CallMeBot] WA_NUMERO e WA_CALLMEBOT_APIKEY precisam estar no .env\n"
                "Siga o SETUP da Opção A no topo deste arquivo."
            )
...
class EvolutionAPINotifier:
    ...
    def __init__(self):
        self.numero    = _ler_env("WA_NUMERO")
        self.base_url  = _ler_env("WA_EVOLUTION_URL", "http://localhost:8080").rstrip("/")
        self.instance  = _ler_env("WA_EVOLUTION_INSTANCE", "radio-tjrn")
        self.apikey    = _ler_env("WA_EVOLUTION_APIKEY")

        if not self.numero or not self.apikey:
            raise ValueError(
                "[EvolutionAPI] WA_NUMERO e WA_EVOLUTION_APIKEY precisam estar no .env\n"
                "Siga o SETUP da Opção B no topo deste arquivo."
            )
    ...
    def enviar(self, mensagem: str) -> bool:
        ...
        # Número no formato internacional sem + (ex: 5584999999999)
...
class NotificadorWhatsApp:
    ...
    def __init__(self):
        modo = _ler_env("WA_MODO", "callmebot").lower()
        self._modo = modo
...
# ─────────────────────────────────────────────
#
# No agente_ia.py, adicione no topo:
#
#   try:
...
#       WA_ATIVO = True
#   except Exception as e:
#       print(f"[AVISO] Notificador WhatsApp não disponível: {e}")
#       WA_ATIVO = False
#
# E nos pontos de evento, por exemplo em run_agent_once():
#
#   # Logo após verificar que o Drive está offline:
#   if WA_ATIVO: notificador.notificar_drive_offline()
#
#   # Ao final de executar_pipelines():
#   if WA_ATIVO:
#       notificador.notificar_relatorio_diario({
#           "boletins": {"ok": boletins_ok, "count": n_boletins, "duracao_s": t_boletins},
#           "njud":     {"ok": njud_ok,     "count": n_njud,     "duracao_s": t_njud},
#           "giro":     {"ok": giro_ok,     "count": n_giro,     "duracao_s": t_giro},
#           "conflitos_corrigidos": conflitos,
#           "duracao_total_s": time.time() - ts_inicio,
#       })
#
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# TESTE RÁPIDO (rode diretamente para testar)
# ─────────────────────────────────────────────
